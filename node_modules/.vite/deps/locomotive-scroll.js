...
function t() {
  return t = Object.assign ? Object.assign.bind() : function(t2) {
    for (var e3 = 1; e3 < arguments.length; e3++) {
      var i3 = arguments[e3];
      for (var s3 in i3) Object.prototype.hasOwnProperty.call(i3, s3) && (t2[s3] = i3[s3]);
    }
    return t2;
...
var i = class {
  advance(t2) {
    ...
    let s3 = false;
    if (this.lerp) this.value = (o3 = this.value, n3 = this.to, (1 - (l3 = 1 - Math.exp(-60 * this.lerp * t2))) * o3 + l3 * n3), Math.round(this.value) === this.to && (this.value = this.to, s3 = true);
    else {
      this.currentTime += t2;
...
var o = class {
  ...
  emit(t2, ...e3) {
    ...
    for (let t3 = 0, s3 = i3.length; t3 < s3; t3++) i3[t3](...e3);
  }
  on(t2, e3) {
    var i3;
...
var l = class {
  constructor({ wrapper: e3 = window, content: l3 = document.documentElement, wheelEventsTarget: r2 = e3, eventsTarget: h2 = r2, smoothWheel: a2 = true, smoothTouch: c2 = false, syncTouch: u = false, syncTouchLerp: p = 0.1, __iosNoInertiaSyncTouchLerp: d = 0.4, touchInertiaMultiplier: m = 35, duration: v, easing: g = (t2) => Math.min(1, 1.001 - Math.pow(2, -10 * t2)), lerp: S = !v && 0.1, infinite: w = false, orientation: f = "vertical", gestureOrientation: y = "vertical", touchMultiplier: T = 1, wheelMultiplier: z = 1, normalizeWheel: _ = false, autoResize: M = true } = {}) {
    this.onVirtualScroll = ({ deltaX: e4, deltaY: i3, event: s3 }) => {
      if (s3.ctrlKey) return;
      const o3 = s3.type.includes("touch"), n3 = s3.type.includes("wheel");
      if ("both" === this.options.gestureOrientation && 0 === e4 && 0 === i3 || "vertical" === this.options.gestureOrientation && 0 === i3 || "horizontal" === this.options.gestureOrientation && 0 === e4 || o3 && "vertical" === this.options.gestureOrientation && 0 === this.scroll && !this.options.infinite && i3 <= 0) return;
      ...
      if (this.isSmooth = (this.options.smoothTouch || this.options.syncTouch) && o3 || this.options.smoothWheel && n3, !this.isSmooth) return this.isScrolling = false, void this.animate.stop();
      s3.preventDefault();
      let r3 = i3;
  ...
  scrollTo(t2, { offset: i3 = 0, immediate: s3 = false, lock: o3 = false, duration: n3 = this.options.duration, easing: l3 = this.options.easing, lerp: r2 = !n3 && this.options.lerp, onComplete: h2 = null, force: a2 = false, programmatic: c2 = true } = {}) {
    if (!this.isStopped && !this.isLocked || a2) {
      ...
      if ("number" == typeof t2) {
        if (t2 += i3, t2 = Math.round(t2), this.options.infinite ? c2 && (this.targetScroll = this.animatedScroll = this.scroll) : t2 = e(0, t2, this.limit), s3) return this.animatedScroll = this.targetScroll = t2, this.setScroll(this.scroll), this.reset(), void (null == h2 || h2(this));
        if (!c2) {
...
var l2 = class {
  ...
  onRender({ currentScroll: t2, smooth: s3 }) {
    ...
    if (this.currentScroll = t2, this._computeProgress(), this.attributes.scrollSpeed && !isNaN(this.attributes.scrollSpeed)) if (this.attributes.scrollEnableTouchSpeed || s3) {
      if (this.isInFold) {
  ...
  _computeProgress(t2) {
    ...
    if (this.progress = s3, s3 != this.lastProgress) {
      if (this.lastProgress = s3, this.attributes.scrollCssProgress && this._setCssProgress(s3), this.attributes.scrollEventProgress && this._setCustomEventProgress(s3), this.attributes.scrollModuleProgress) for (const t3 of this.progressModularModules) this.modularInstance && this.modularInstance.call("onScrollProgress", s3, t3.moduleName, t3.moduleId);
...
var o2 = class {
  ...
  onResize({ currentScroll: t2 }) {
    for (const s3 of this.RAFScrollElements) s3.onResize({ currentScroll: t2 });
  }
  onRender({ currentScroll: t2, smooth: s3 }) {
    for (const e3 of this.scrollElementsToUpdate) e3.onRender({ currentScroll: t2, smooth: s3 });
  }
  ...
  _checkRafNeeded(t2) {
    ...
    e3("scrollSpeed");
    for (const e4 of s3) if (e4 in t2.dataset) return true;
    return false;
  }
};
var a = class {
